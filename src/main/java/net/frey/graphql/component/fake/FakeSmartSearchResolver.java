...
public class FakeSmartSearchResolver {
    ...
    public List<SmartSearchResult> getSmartSearch(
            @InputArgument(name = QUERY.SMARTSEARCH_INPUT_ARGUMENT.Keyword) String keyword) {
        ...
        if (keyword == null) {
            ...
            HELLO_LIST.stream()
                    .filter(hello -> containsIgnoreCase(hello.getText(), keyword))
                    .forEach(smartSearchList::add);

            BOOK_LIST.stream()
                    .filter(book -> containsIgnoreCase(book.getTitle(), keyword))
                    .forEach(smartSearchList::add);
        }
